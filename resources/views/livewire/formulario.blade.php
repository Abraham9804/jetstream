<div>
    ...
    <!-- Modal for Edit Form -->
    @if($openEdit)
        <div class="bg-gray-800 bg-opacity-25 fixed inset-0" wire:click="$set('openEdit', false)">
            <div class="py-12">
                <div class="max-w-5xl mx-auto sm:px-6 lg:px-8" wire:click.stop>
                    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                        <div class="bg-white shadow rounded-lg p-6">
                            ...
                            <form wire:submit="update" novalidate>
                                <div>
                                    <x-label for="title" value="Titulo" />
                                    <x-input id="title" type="text" class="mt-1 block w-full" autofocus wire:model="postEdit.title" />
                                </div>
                                <div>
                                    <x-label for="content" value="Contenido" />
                                    <x-textarea id="content" class="mt-1 block w-full" rows="3" wire:model="postEdit.content">
                                        {{ old('content') }}
                                    </x-textarea>
                                </div>
                                <div>
                                    <x-label for="category_id" value="Categoria" />
                                    <x-select id="category_id" class="mt-1 block w-full" wire:model="postEdit.category_id">
                                        <option value="" disabled>Select Category</option>
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}">{{ $category->name }}</option>
                                        @endforeach
                                ...
                                <div>
                                    ...
                                    <ul>
                                        @foreach($tags as $tag)
                                            <li>
                                                <label for="{{$tag->id}}" value="{{$tag->name}}">
                                                    <x-checkbox id="{{$tag->id}}" value="{{$tag->id}}" wire:model="postEdit.tags"/>
                                                    {{$tag->name}}
                                                </label>
                                            </li>
                                        @endforeach
